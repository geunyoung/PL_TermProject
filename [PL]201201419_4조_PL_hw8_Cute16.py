    ...
    def run_func(self, func_node):
        ...
        if func_node.type is TokenType.CAR:
            ...
            if result.next is None:
                return create_quote_node(None)

            return create_quote_node(result.next, True)

        elif func_node.type is TokenType.PLUS:

            rhs1 = pop_node_from_quote_list(self.run_expr(rhs1)).value
            rhs2 = pop_node_from_quote_list(self.run_expr(rhs2)).value

            return Node(TokenType.INT, int(rhs1)+int(rhs2))
            ...
            if not is_quote_list(expr_rhs1):
                result = create_quote_node(pop_node_from_quote_list(expr_rhs1)).value.next
                print result
                result.next = pop_node_from_quote_list(expr_rhs2)
            else:
                result = create_quote_node(pop_node_from_quote_list(expr_rhs1),True).value.next
                print  result
                result.next = pop_node_from_quote_list(expr_rhs2)
            ...
            if rhs1.type | rhs2.type is not TokenType.INT: return self.FALSE_NODE
            elif pop_node_from_quote_list(rhs1).value is pop_node_from_quote_list(rhs2).value :
                    return self.TRUE_NODE
            else:
                    return self.FALSE_NODE
            ...
            while rhs1 is not None:
                if rhs1.value.type is TokenType.LIST:
                    if self.run_expr(pop_node_from_quote_list(rhs1.value)).type is TokenType.TRUE:
                        return rhs1.value.next
                    else:
                        rhs1 = rhs1.next
                else:
                    if rhs1.value.type is TokenType.TRUE:
                        return rhs1.value.next
                    else:
                        rhs1 = rhs1.next
